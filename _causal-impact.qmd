# And now...to Bayesian Things {background-color="#065CBC7C"}

# Specifically, Bayesian Structural Time-Series {background-color="#065CBC7C"}

# Think "time-series decomposition turned up to 11" {background-color="#065CBC7C"}

## Mark Edmondson built a tool using this [7 years ago]{.underline}! {background-color="#065CBC7C"}

![](images/ga_effect.png)

## At its core: estimate the impact of an intervention {background-color="#065CBC7C"}

## "We didn't test it, so can we just do a pre-/post- analysis?" {background-color="#065CBC7C"}
...
# Let's put [covariates]{.underline} to use. {background-color="#065CBC7C"}

## Two criteria for any covariates {background-color="#065CBC7C"}

1. Their relationship to the metric of interest is stable
2. They are not themselves affected by the intervention

## Covariates require [critical thought]{.underline} and [creativity]{.underline} {background-color="#065CBC7C"}

Sessions
Housing starts
Google Trends

## Back to our original example {background-color="#065CBC7C"}

This is without any covariates included.

(Include statistical measures...)

## Let's add a covariate {background-color="#065CBC7C"}

And, with one really solid covariate.

(Include statistical measures...)

# This is not a silver bullet! {background-color="#065CBC7C"}
